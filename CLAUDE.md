...
- **`view/`** — the **OpenCV surface**, structured to the `final_plan.md` blueprint (State / Strategy / DI, a coordinator + focused renderers). Like every consumer it holds **no chess and no timing**: it reads the board via `GameEngine.snapshot()`/`pieces()`/`active_motions()`, sends input via the injected `Controller`, and draws. `view/` imports only `model` (the boundary test — now recursive over subpackages — enforces it); it never reads `config`.
  - `img.py` — the given cv2 primitive (load/resize/alpha-blend/put-text, plus `draw_rect` and **`fill_rect`**, the translucent filled counterpart used by the overlays — it blends colour channels only, leaving the frame's own alpha intact, and clips instead of raising).
  ...
  - `rendering/` — the **coordinator + renderer split** (final_plan §7.2a). `view_state.py`: **`ViewState`** — everything one frame needs (`snapshot`/`motions`/`rests`/`selected`/`targets`) as one frozen value, assembled by `GameLoop` from the engine + controller, so a new thing to draw adds a field instead of another positional argument threaded through the loop, the coordinator and every fake. `board_view.py`: `BoardView` draws nothing itself, just calls its collaborators in order (and composes the optional panel last); `render(state, dt_ms)`. `board_renderer.py`: the background template (forced BGRA), a fresh copy per frame. The artwork is **tiled, not stretched**: it is scaled so one *depicted* square equals one cell (`assets.board_image_cells` says how many it depicts), then repeated and cropped to the board — so a board that is not the artwork's size still gets squares that line up with its cells. On the standard 8x8 board the result is byte-identical to a plain stretch. `piece_renderer.py`: composites pieces, keeps a per-piece `AnimatedSprite` (keyed by piece id **and current kind** — a promotion mutates `kind` on the same piece object, so an id-only key would keep drawing the pawn forever; the kind in the key retires the old sprite and builds the queen's), **glides** in-flight pieces via `MotionView.progress` and maps `PieceState` → sprite-state folder (config-driven). `overlay_renderer.py`: selection highlight, **legal-move targets** (translucent green on the cells the rules returned), the **cooldown countdown** (a translucent yellow square that drains from the top down, so the yellow left is the time left) + game-over banner. It decides nothing it draws — targets and rest fractions arrive already computed on the `ViewState`. `panel_renderer.py`: composes the board frame into a **wider canvas** and draws the side panel from the observers — **each player in their own column** (black at the top, white at the bottom), the **score gap** between them (`ScoreObserver.lead`) in the middle, and a **per-side moves log**, each entry stamped with its server-time and written with an `x` on a capture (e.g. `12.3s  R a1xa4`). `renderer.py`: the `Renderer` protocol + `InputEvent` (`CLICK`/`JUMP`/`QUIT`). `cv2_renderer.py`: the one concrete `Renderer` — a **resizable** window (`WINDOW_NORMAL | WINDOW_KEEPRATIO`). **Mouse coordinates arrive already in canvas (image) pixels** — highgui undoes the window's zoom before calling the callback — so the renderer passes them through untouched. Rescaling them by the window size (e.g. via `getWindowImageRect`, which reports the *client area*, not the letterboxed image rect) applies the zoom twice and lands clicks on the wrong square. It reports QUIT on <kbd>Esc</kbd> **and** when the window is closed with its X button — cv2 sends no close event and its next `imshow` silently recreates a dismissed window, so `_was_dismissed` polls `WND_PROP_VISIBLE` instead.
  - `events/` — the **Observer pipeline** (final_plan §7.6) for the side panel. `event_bus.py`: `EventBus` (subscribe/publish by type). `events.py`: `MoveResolved` (color + text + **`at_ms` server-time**)/`CaptureResolved`/**`PromotionResolved`** (color + `new_kind` — a kind change scores on its own, so it is a distinct event) (plain UI data). `settlement_detector.py`: `SettlementDetector` **diffs consecutive snapshots** and publishes those events — the adaptation to our engine, which (unlike final_plan's) has no settlement hook, so nothing in `engine/`/`model/` changes. It is handed the server clock each `observe(snapshot, now_ms)` (it never reads a clock itself), stamps each move with it, writes an `x` when the move lands on a cell a piece just vanished from (a capture), and emits a `PromotionResolved` when a piece's kind changed. `observers/`: `ScoreObserver` (config-driven piece values; `on_capture` + **`on_promotion`** credit points, `lead` is the signed white−black gap) and `MovesLogObserver` (**per-side** `entries["white"]`/`entries["black"]`, each `(at_ms, text)`), subscribed once at the composition root.
...
- **`composition/app_factory.py`** — the composition root. Builds the codec, parser, printer, rule set, arbiter (with the collision resolver), engine, controller and command table from a `GameConfig`, plus `build_gui_app` and its helpers `build_board_view`/`build_piece_folders`/`build_state_folders`/`build_scoreboard`/`build_speed_for` — `build_piece_folders` is the **one place** the on-disk piece naming (`<symbol><suffix>`, e.g. `QW`/`KB`) is spelled out, `build_scoreboard(cfg, white_name, black_name)` wires the panel's Observer pipeline (bus + observers + snapshot detector), and `build_speed_for` is the **one place** a piece's kind is turned into a per-cell duration for the arbiter (so `realtime/` stays kind-blind). `build_gui_app` takes optional `white_name`/`black_name` overrides (from the GUI's CLI), defaulting to the configured player names. **The swap point for everything**: a different rule set, time model, board or surface is a change here and nowhere else. Notably it contains *no list of pieces* — that comes from config.
- **`config.py` / `config.toml`** — `load(path=None) -> GameConfig` **returns an immutable value** (it does not populate module globals; a config is injected, not reached into). `GameConfig` holds `cell_size`, `ms_per_cell`, `jump_duration_ms`, `long_rest_ms`, `short_rest_ms`, `empty_token`, `pieces: tuple[PieceSpec]`, and `assets: AssetsConfig` (the GUI's `board_image`/`pieces_dir`/`default_board` paths — resolved absolute against the package — plus `board_image_cells` (how many cells the board artwork depicts, so it can be tiled to any board size) and the `white_suffix`/`black_suffix` and `idle`/`move`/`jump` state-folder names, so re-skinning is config only). Also `white_player`/`black_player` (panel names), `panel_width` (0 hides the side panel). A `PieceSpec` is `name`/`symbol`/`movement`/`directions`/`offsets`/`promotes_to`/`victory_on_capture`/`flies_over` (the knight's collision exemption)/`value` (score-panel point value)/**`ms_per_cell`** (this piece's own per-cell travel time — a slow "drone" — `None` meaning "use the global speed"). **Add new tunables to `config.toml` and map them in `config.load`, never as literals in code.**
...
- **`gui.py`** — `main(argv=None)`, the graphical entry point (`python -m kongfuchess.gui [board-file] [--white NAME] [--black NAME]`; with no board argument it loads `assets.default_board`, with no name arguments the configured `[players]` names). Reads config, parses the board, and hands off to `app_factory.build_gui_app(...).run()`. The `--white`/`--black` names are the easy, forward-compatible hook for setting players — a name-entry screen later fills in the same two arguments.
...
- **Still missing vs. full Kung-Fu-Chess**: no networking, no castling. (Parallel real-time moves, post-move **cooldown** (long-rest) and post-jump short-rest, **per-piece speed**, the **collision model** (§1 arrival table / §2 mid-path / §3 two-movers, knight exempt), jump/dodge, pawn two-step, promotion, the **OpenCV GUI** with gliding pieces, and the **side panel** with per-player columns / server-timestamped, capture-marked moves log / score with the gap between sides / promotion bonus **are** implemented.)
...
`kong_fu_chess_requirements.md` is the full product spec (Hebrew): Kung-Fu-Chess is meant to be a **real-time, no-turns** game where moves take physical travel time, pieces have a post-move cooldown, and you win only by actually capturing the enemy king (no check/checkmate). **The current code is a layered rebuild** implementing real-time movement over time (the `realtime/` layer), post-move/post-jump cooldowns, per-piece speed, a configurable victory condition, jump/dodge, pawn two-step and promotion, config-driven pieces, a text surface, and an OpenCV surface with a scoring/moves-log side panel. Still **simplified**: no networking. When extending, honor the spec's design principle: make the *known* future extensions (new piece types, new rule sets, new commands, a `view/` renderer, a binary board) easy, but do **not** add speculative abstractions for things not yet needed.
