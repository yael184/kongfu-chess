...
- **`config.py` / `config.toml`** — `load(path=None) -> GameConfig` **returns an immutable value** (it does not populate module globals; a config is injected, not reached into). `GameConfig` holds `cell_size`, `ms_per_cell`, `jump_duration_ms`, `long_rest_ms`, `short_rest_ms`, `empty_token`, `pieces: tuple[PieceSpec]`, and `assets: AssetsConfig` (the GUI's `board_image`/`pieces_dir`/`default_board` paths — resolved absolute against the package — plus the `white_suffix`/`black_suffix` and `idle`/`move`/`jump` state-folder names, so re-skinning is config only). Also `white_player`/`black_player` (panel names), `panel_width` (0 hides the side panel). A `PieceSpec` is `name`/`symbol`/`movement`/`directions`/`offsets`/`promotes_to`/`victory_on_capture`/`flies_over` (the knight's collision exemption)/`value` (score-panel point value). **Add new tunables to `config.toml` and map them in `config.load`, never as literals in code.**
