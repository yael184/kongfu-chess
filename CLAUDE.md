...
- **`view/`** — the **OpenCV surface**, and like every consumer it holds **no chess and no timing**: it reads the board via `GameEngine.snapshot()`/`pieces()`, sends input via the injected `Controller`, and draws. `img.py` is the given low-level primitive (load/resize/alpha-blend/put-text over cv2). `animation.py`: `Animation.frame_at(now)` — a state's frames + `frames_per_sec`/`is_loop`, so frame choice is a pure function of the clock. `sprite_repository.py`: `SpriteRepository` owns the **on-disk asset layout** (`states/<state>/sprites/<n>.png` + `config.json`) exactly as `text_io` owns the token format; it is handed a resolved folder per `(kind, color)` and never names a piece. `board_renderer.py`: `BoardRenderer.render(snapshot, motions, airborne, selected, now)` composes one frame — background, idle/jumping pieces, **gliding** pieces (interpolated with `MotionView.progress`, drawn instead of the logical source cell), selection highlight, game-over banner. `game_loop.py`: `GameLoop` is the real-time driver (the visual sibling of `ScriptRunner`) — it measures elapsed wall time, calls `engine.wait(dt)`, turns left/right clicks into `controller.handle_click`/`handle_jump`, and shows frames until the window closes. `view/` imports only `model` (the boundary test enforces it) — it never reads `config`.
- **`composition/app_factory.py`** — the composition root. Builds the codec, parser, printer, rule set, arbiter, engine, controller and command table from a `GameConfig`, plus `build_gui_app` (the graphical surface) and its `build_renderer`/`build_piece_folders` — the latter is the **one place** the on-disk piece naming (`<symbol><suffix>`, e.g. `QW`/`KB`) is spelled out. **The swap point for everything**: a different rule set, time model, board or surface is a change here and nowhere else. Notably it contains *no list of pieces* — that comes from config.
- **`config.py` / `config.toml`** — `load(path=None) -> GameConfig` **returns an immutable value** (it does not populate module globals; a config is injected, not reached into). `GameConfig` holds `cell_size`, `ms_per_cell`, `jump_duration_ms`, `empty_token`, `pieces: tuple[PieceSpec]`, and `assets: AssetsConfig` (the GUI's `board_image`/`pieces_dir`/`default_board` paths — resolved absolute against the package — plus the `white_suffix`/`black_suffix` and `idle`/`move`/`jump` state-folder names, so re-skinning is config only). A `PieceSpec` is `name`/`symbol`/`movement`/`directions`/`offsets`/`promotes_to`/`victory_on_capture`. **Add new tunables to `config.toml` and map them in `config.load`, never as literals in code.**
...
- **Still missing vs. full Kung-Fu-Chess**: no post-move **cooldown**, no scoring, no networking. (Jump/dodge, pawn two-step, promotion, and the **OpenCV GUI** with gliding pieces **are** implemented.) The sprite assets ship `short_rest`/`long_rest` (cooldown) states the engine does not model yet — the renderer only uses `idle`/`move`/`jump`; wiring the rest states is part of adding cooldown, not a view change.
