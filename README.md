...
| [view/](kongfuchess/view/) | The **OpenCV surface** (State/Strategy/Observer/DI per `final_plan.md`): `sprites/` (`SpriteState`/`AnimatedSprite`/`SpriteLibrary`), `rendering/` (a `BoardView` coordinator over `BoardRenderer`/`PieceRenderer`/`OverlayRenderer`/`PanelRenderer`, plus a resizable `Cv2Renderer`), `events/` (an `EventBus` fed by a snapshot-diff `SettlementDetector`, driving the score/moves-log observers), and the real-time `GameLoop`. Holds no chess and no timing; reads the engine snapshot and drives the injected `Controller` |
