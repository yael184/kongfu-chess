...
def test_valid_move_is_ok():
    ...
    result = RuleEngine(build_registry(config.load().pieces)).validate_move(board, Position(0, 0), Position(0, 2))
...
def test_destination_outside_board_is_rejected():
    rook = pc("r", Color.WHITE, PieceKind.ROOK, 0, 0)
    board = board_with(3, 3, rook)
    result = RuleEngine(build_registry(config.load().pieces)).validate_move(board, Position(0, 0), Position(0, 5))
...
def test_source_outside_board_is_rejected():
    ...
    result = RuleEngine(build_registry(config.load().pieces)).validate_move(board, Position(9, 9), Position(0, 0))
    assert result.reason == REASON_OUTSIDE_BOARD


def test_empty_source_is_rejected():
    board = board_with(3, 3)
    result = RuleEngine(build_registry(config.load().pieces)).validate_move(board, Position(0, 0), Position(0, 1))
    assert result.is_valid is False
...
def test_illegal_piece_move_is_rejected():
    ...
    result = RuleEngine(build_registry(config.load().pieces)).validate_move(board, Position(0, 0), Position(2, 2))
...
def test_rule_engine_does_not_mutate_the_board():
    ...
    enemy = pc("e", Color.BLACK, PieceKind.PAWN, 0, 2)
    board = board_with(3, 3, rook, enemy)
    RuleEngine(build_registry(config.load().pieces)).validate_move(board, Position(0, 0), Position(0, 2))
